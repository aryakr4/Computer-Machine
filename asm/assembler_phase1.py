...
ASM_MEMOP_PAT = re.compile(r"""
   ...
   \s*
    ...
    \s+
    (?P<target>    r[0-9]+),            # Target register
    (?P<labelref>  [a-zA-Z]\w*)  #like label but without ':' and required 
   # Optional comment follows # or ; 
   (
     \s*
     (?P<comment>[\#;].*)
...
def transform(lines: list[str]) -> list[str]:
    ...
    for lnum in range(len(lines)):
        ...
        try:
            ...
            error_count += 1
            print(f"Syntax error in line {lnum}: {e}", file=sys.stderr)
...
def cli() -> object:
    ...
    parser.add_argument("sourccdefile", type=argparse.FileType('r'),
